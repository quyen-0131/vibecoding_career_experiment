...
function ToggleList({ activities, selected, limit, onChange, badge }: { activities: NormalizedActivity[]; selected: string[]; limit: number; onChange: (ids: string[]) => void; badge: string }) {
  return <div className="priority-list">{activities.map((activity) => {
    const active = selected.includes(activity.id);
    return <button className={active ? "priority-card selected" : "priority-card"} type="button" key={activity.id} aria-pressed={active} disabled={!active && selected.length === limit} onClick={() => onChange(active ? selected.filter((id) => id !== activity.id) : [...selected, activity.id])}><span className="check" aria-hidden="true">{active ? "✓" : ""}</span><span><strong>{activity.label}</strong><small>{badge}</small></span></button>;
  })}</div>;
}
...
export function PrioritySelectionScreen({ activities, responses, priorities, minimized, onPrioritiesChange, onMinimizedChange, onContinue, onBack }: Props) {
  ...
  return (
    <section className="screen wide-screen">
      ...
      <p className="lead compact">{needsForcedChoice ? "Highlight five to eight types of work you most want in your future career." : "We&apos;ll carry the activities you want more of into your evidence map."}</p>
      {moreActivities.length ? <><div className="priority-section-heading"><h2>Activities you most want more of</h2><span>{effectivePriorities.length}{needsForcedChoice ? ` selected · choose ${MIN_PRIORITY_ACTIVITIES}–${MAX_PRIORITY_ACTIVITIES}` : " selected"}</span></div><ToggleList activities={moreActivities} selected={effectivePriorities} limit={MAX_PRIORITY_ACTIVITIES} onChange={needsForcedChoice ? onPrioritiesChange : () => {}} badge="More" /></> : <div className="empty-priority"><strong>No activities marked “More”</strong><p>That is still useful evidence. No priority activity is required.</p></div>}
      {lessActivities.length >= 2 && <section className="minimize-section"><div className="priority-section-heading"><div><h2>Which activities would you most want to minimise?</h2><p>Optional. Choose up to three.</p></div><span>{validMinimized.length} of 3 selected</span></div><ToggleList activities={lessActivities} selected={validMinimized} limit={3} onChange={onMinimizedChange} badge="Less" /></section>}
      <div className="actions"><button className="button ghost" type="button" onClick={onBack}>Back to preference review</button><button className="button primary" type="button" disabled={!canContinue} onClick={() => { if (!needsForcedChoice) onPrioritiesChange(effectivePriorities); onContinue(); }}>See my evidence map <span aria-hidden="true">→</span></button></div>
    </section>
  );
}